...
class MultiProvider:
    ...
    PROVIDER_NAMES = ("openai", "gemini", "anthropic", "groq", "openrouter")

    def __init__(self) -> None:
        configured = os.getenv("SAIT_PROVIDER_ORDER") or os.getenv("SAIT_PROVIDERS", "openai,gemini,anthropic,groq,openrouter")
        self.providers = [p.strip().lower() for p in configured.split(",") if p.strip() and p.strip().lower() in self.PROVIDER_NAMES]
    ...
    def set_provider(self, provider: str) -> None:
        ...
        if provider not in {"auto", *self.PROVIDER_NAMES}:
    ...
    def available_providers(self) -> list[str]:
        ...
        return [p for p in self.providers if os.getenv(keys[p])]
    ...
    def _openai(self, messages, tools, previous_response_id):
        ...
        model = os.getenv("OPENAI_MODEL", os.getenv("SAIT_MODEL", "gpt-5.6"))
    ...
    def _discover_gemini_model(self, api_key: str) -> str:
        ...
        response = httpx.get("https://generativelanguage.googleapis.com/v1beta/models", params={"key": api_key, "pageSize": 100}, timeout=30)
        ...
        for item in models:
            ...
            if "generateContent" not in methods or not name: continue
            short = name.split("/", 1)[-1]
            if configured and short == configured: return short
            lower = short.lower(); score = 0
        ...
        if not candidates: raise RuntimeError("Gemini API key has no model available with generateContent.")
    ...
    def _gemini(self, messages, tools):
        ...
        for msg in messages:
            role = msg.get("role"); text = str(msg.get("content", ""))
            if role == "system": system_parts.append(text)
            elif role == "user": contents.append({"role": "user", "parts": [{"text": text}]})
            elif role == "assistant": contents.append({"role": "model", "parts": [{"text": text}]})
            elif msg.get("type") == "function_call_output": contents.append({"role": "user", "parts": [{"functionResponse": {"name": msg.get("name", "tool"), "response": {"result": msg.get("output", "")}}}]})
        body: dict[str, Any] = {"contents": contents, "generationConfig": {"temperature": 0.2}}
        if system_parts: body["systemInstruction"] = {"parts": [{"text": "\n".join(system_parts)}]}
        if tools:
            declarations = []
            for t in tools:
                schema = dict(t.get("parameters") or {"type": "object", "properties": {}}); schema.pop("additionalProperties", None)
                declarations.append({"name": t["name"], "description": t.get("description", ""), "parameters": schema})
            body["tools"] = [{"functionDeclarations": declarations}]
        response = httpx.post(f"https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent", params={"key": api_key}, json=body, timeout=120)
        response.raise_for_status(); data = response.json()
        candidates = data.get("candidates") or []; parts = ((candidates[0].get("content") or {}).get("parts") if candidates else []) or []
        texts, calls, output_items = [], [], []
        for part in parts:
            if part.get("text"): texts.append(part["text"])
            fc = part.get("functionCall")
            if fc:
                call_id = f"gemini-{len(calls)+1}"; args = fc.get("args") or {}
    ...
    def _compatible(self, provider, messages, tools):
        ...
        response.raise_for_status(); return self._normalize_chat_response(response.json(), provider, model)
    ...
    def _anthropic(self, messages, tools):
        ...
        response.raise_for_status(); data = response.json(); calls, output_items, texts = [], [], []
    ...
    def complete(self, messages, tools=None, previous_response_id=None):
        errors = []
        candidates = [self.active_provider] if self.active_provider != "auto" else self.providers
        for provider in candidates:
            if provider == "auto" or not self._configured(provider): continue
            try:
                ...
                if self.active_provider != "auto": break
                time.sleep(0.2)
