...
class MultiProvider:
    """Provider pool for OpenAI, Gemini, Anthropic, Groq and OpenRouter."""

    PROVIDER_NAMES = ("openai", "gemini", "anthropic", "groq", "openrouter")

    def __init__(self) -> None:
        configured = os.getenv("SAIT_PROVIDER_ORDER") or os.getenv(
            "SAIT_PROVIDERS", "openai,gemini,anthropic,groq,openrouter"
        )
        self.providers = [
            p.strip().lower()
            for p in configured.split(",")
            if p.strip() and p.strip().lower() in self.PROVIDER_NAMES
        ]
                ...
                for content in getattr(item, "content", []) or []:
                    ...
                    if text:
                        ...
                        content_items.append({
                            "type": getattr(content, "type", "output_text"),
                            "text": text,
                        })
                if content_items:
                    output_items.append({
    ...
    def _gemini(self, messages, tools):
        ...
        for msg in messages:
            ...
            if role == "system":
                ...
                contents.append({
                    "role": "model",
                    "parts": [{"functionCall": {"name": msg.get("name", ""), "args": args}}],
                })
            elif msg.get("type") == "function_call_output":
                contents.append({
                    "role": "user",
                    "parts": [{"functionResponse": {
                        ...
                        "response": {"result": msg.get("output", "")},
                    }}],
                })
    ...
    def _anthropic(self, messages, tools):
        ...
        for item in data.get("content", []):
            if item.get("type") == "text":
                ...
                call_id = item.get("id")
                args = item.get("input", {})
                calls.append({"name": item.get("name"), "call_id": call_id, "arguments": args})
                output_items.append({
                    "type": "function_call",
                    "name": item.get("name"),
                    "call_id": call_id,
    ...
    def complete(self, messages, tools=None, previous_response_id=None):
        ...
        for provider in candidates:
            if provider == "auto" or not self._configured(provider):
                continue
            try:
                if provider == "openai":
                    result = self._openai(messages, tools)
                elif provider == "gemini":
                    result = self._gemini(messages, tools)
